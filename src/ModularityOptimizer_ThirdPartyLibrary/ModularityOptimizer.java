package ModularityOptimizer_ThirdPartyLibrary;
/**
 * ModularityOptimizer
 *
 * @author Ludo Waltman
...
public class ModularityOptimizer
{
    public static void main(String[] args) throws IOException
    {
        ...
        if (args.length == 9)
        {
            inputFileName = args[0];// input file directory 
            outputFileName = args[1];// output file directory
            modularityFunction = Integer.parseInt(args[2]);// 1: standard; 2: al
            resolution = Double.parseDouble(args[3]);//Resolution parameter (e.g., 1.0)
            algorithm = Integer.parseInt(args[4]);// three types of algorithms to try 
            nRandomStarts = Integer.parseInt(args[5]);// number of random starts, e.g., 10
            nIterations = Integer.parseInt(args[6]);// number of iteractions, e.g., 10.
            randomSeed = Long.parseLong(args[7]);//random seed, e.g., 0.
            printOutput = (Integer.parseInt(args[8]) > 0);//print the result?
