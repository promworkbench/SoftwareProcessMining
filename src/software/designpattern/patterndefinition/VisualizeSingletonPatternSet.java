...
public class VisualizeSingletonPatternSet {
	//the pattern instance to dot mapping
		...
		@Plugin(name = "Visualize Singleton Pattern Instances", 
				...
				@PluginVariant(variantLabel = "Visualize Singleton Pattern Instances", 
						requiredParameterLabels = {0})// it needs one input parameter
		
		public JComponent visualizeTop(PluginContext context, SingletonPatternSet patternSet)
		{
			...
			{
				Dot dot = convertInstance2DOT((SingletonPatternClass)instance);
		...
		public static Dot convertInstance2DOT(SingletonPatternClass pattern)
		{
			Dot dot = new Dot();
			dot.setDirection(GraphDirection.leftRight);
		
			// prepare the table
			String Singleton= "Singleton Class: "+pattern.getSingletonClass();
			String getInstance ="getInstance Method: "+pattern.getGetInstanceMethod();
			String traceInvocation= "Run/Invocation Count: "+pattern.getTraceNumber()+"/"+pattern.getInvocationNumber();
			
			DotNode tab = dot.addNode("{"+Singleton+"}|"+"{"+getInstance+"}|"+"{"+traceInvocation+"}");
