...
/**
 * this plug-in is used to discover a class interaction graph from software event log. 
 * The interaction relation is infered by method calling relation. 
 * @author cliu3
 */
...
public class ClassInteractionGraphPlugin {
	...
	public ClassInteractionGraph cigDiscovery(UIPluginContext context, XLog softwareLog)
	{
		...
		context.getFutureResult(0).setLabel("Class Interaction Graph discovered from: "+ XConceptExtension.instance().extractName(softwareLog));
		...
		for (XTrace trace: softwareLog)
		{
			for(XEvent event:trace)
			{
				if (XSoftwareExtension.instance().extractCallerclass(event).equals("null"))
				{
					...
					vertexSet.add(XSoftwareExtension.instance().extractPackage(event)+"."+XSoftwareExtension.instance().extractClass(event));
				}
				else
				{				
					vertexSet.add(XSoftwareExtension.instance().extractPackage(event)+"."+XSoftwareExtension.instance().extractClass(event));
					vertexSet.add(XSoftwareExtension.instance().extractCallerpackage(event)+"."+XSoftwareExtension.instance().extractCallerclass(event));
					Edge e = new Edge(XSoftwareExtension.instance().extractCallerpackage(event)+"."+XSoftwareExtension.instance().extractCallerclass(event), 
							XSoftwareExtension.instance().extractPackage(event)+"."+XSoftwareExtension.instance().extractClass(event));
