package software.processmining.imports;
...
@UIImportPlugin(
		description = "Import Component Configuration (.conf)",//show the type in window for recording files
...
public class ImportComponentConfiguration extends AbstractImportPlugin{
	...
	public static ComponentConfig componentConfig(InputStream input)
	{
		...
		try {
			...
			{				
				// each line is composed of one component name and a set of classes, the component name is the first element and the followings are classes
		...
		System.out.println(config.getAllComponents());
		return config;
	}
	
	//get the class part, sample input "CH.ifa.draw.standard.StandardDrawingView", output: StandardDrawingView
	...
	//get the package part sample input "CH.ifa.draw.standard.StandardDrawingView",  output: CH.ifa.draw.standard
